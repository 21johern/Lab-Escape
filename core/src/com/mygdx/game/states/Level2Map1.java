...
public class Level2Map1 extends State{
    ...
    public MapObjects keyObjects;
    public Array<Polygon> keys;

    public Level2Map1(GameStateManager stateManager) {
        super(stateManager);
        ...
        map = new TmxMapLoader().load("lvl2.1.tmx");
        ...
        keyObjects = map.getLayers().get("Keys").getObjects();
        keys = new Array<Polygon>();
        ...
        counter = 0;
        for (PolygonMapObject obj : keyObjects.getByType(PolygonMapObject.class)) {
            float[] vertices = obj.getPolygon().getVertices();
            for (int i = 0; i < vertices.length; i++) {
                vertices[i] = vertices[i] * State.PIXEL_TO_METER;
            }
...
//        cam.update();
    ...
    public void keyCheck(){
        for (Polygon keyCollect: keys){
            if(Intersector.overlapConvexPolygons(player.bounds, keyCollect)){
                if(MyGdxGame.keysHeld == 0){
